...
public final class HomeLoanCalculator {
    ...
    public BigDecimal calculateMonthlyPayment(BigDecimal principalAmount, BigDecimal annualInterestRate, int repaymentPeriod) {
        validateCalculationInputs(principalAmount, annualInterestRate, repaymentPeriod);
    ...
    public BigDecimal calculateRemainingPrincipal(
        ...
        BigDecimal monthlyPaymentAmount = calculateMonthlyPayment(principalAmount, annualInterestRate, repaymentPeriod);
    ...
    public int calculateRepaymentPeriod(BigDecimal principalAmount, BigDecimal annualInterestRate, BigDecimal monthlyPaymentAmount) {
        ...
        Assert.notNull(annualInterestRate, "annualInterestRate must not be null");
        ...
        Assert.isTrue(annualInterestRate.signum() > 0, "annualInterestRate must be positive");
    ...
    public PartialRepaymentCalculation calculatePartialRepayment(
        ...
        return new PartialRepaymentCalculation(
                newPrincipalAmount,
                calculateMonthlyPayment(newPrincipalAmount, annualInterestRate, repaymentPeriod),
                calculateRepaymentPeriod(newPrincipalAmount, annualInterestRate, monthlyPaymentAmount));
    }

    private BigDecimal calculateMonthlyInterestRate(BigDecimal annualInterestRate) {
        return annualInterestRate.divide(ONE_HUNDRED, MATH_CONTEXT).divide(TWELVE, MATH_CONTEXT);
    }

    private void validateCalculationInputs(BigDecimal principalAmount, BigDecimal annualInterestRate, int repaymentPeriod) {
        Assert.notNull(principalAmount, "principalAmount must not be null");
        Assert.notNull(annualInterestRate, "annualInterestRate must not be null");
        Assert.isTrue(principalAmount.signum() > 0, "principalAmount must be positive");
        Assert.isTrue(annualInterestRate.signum() > 0, "annualInterestRate must be positive");
